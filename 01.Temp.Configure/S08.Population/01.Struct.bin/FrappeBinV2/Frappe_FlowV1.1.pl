...
die "Version 1.0\t2010-01-12;\nUsage: $0 <In(add_ref.list)><individual.txt><OutDir><qsubPara>\n"unless (@ARGV==3 || @ARGV ==4 );
...
$ARGV[3]||="q:bc.q_P:paptest";

my $qsubPara= $ARGV[3] ;

my @UO=split(/\_/,$qsubPara);
$qsubPara="";
for (my $ik=0 ; $ik<=$#UO ; $ik++)
{
	my @cc=split /:/,$UO[$ik];
	if ($qsubPara eq "" )
	{
...
system("qsub  -cwd -l vf=1.688G -S  /bin/sh  $qsubPara  F_step1.sh ") ;
...
system ("sed -i 's#cwd#cwd  $qsubPara #g'  F_step2K_qsub.sh ");
